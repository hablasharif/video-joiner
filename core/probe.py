    ...
    try:
        ...
        for stream in data.get("streams", []):
            ...
            if codec_type == "video" and info.video is None:
                ...
                v_info.width = int(stream.get("width", 0) or 0)
                v_info.height = int(stream.get("height", 0) or 0)
                
                # FPS
                avg_fps = stream.get("avg_frame_rate", "0/0")
                r_fps = stream.get("r_frame_rate", "0/0")
                parsed_fps = parse_fps(avg_fps) or parse_fps(r_fps)
                v_info.fps = round(parsed_fps, 2)
                v_info.fps_str = avg_fps if avg_fps not in ("0/0", "") else r_fps
                
                v_info.pix_fmt = stream.get("pix_fmt", "")
                try:
                    v_info.bitrate = int(stream.get("bit_rate", 0) or 0)
                except (ValueError, TypeError):
                    v_info.bitrate = 0
                    
                try:
                    v_dur = float(stream.get("duration", 0.0) or 0.0)
                    if v_dur > 0 and info.duration == 0:
                        info.duration = v_dur
                    v_info.duration = v_dur
                except (ValueError, TypeError):
                    pass
                ...
                try:
                    a_info.sample_rate = int(stream.get("sample_rate", 0) or 0)
                except (ValueError, TypeError):
                    a_info.sample_rate = 0
                try:
                    a_info.channels = int(stream.get("channels", 0) or 0)
                except (ValueError, TypeError):
                    a_info.channels = 0
                a_info.channel_layout = stream.get("channel_layout", "")
                try:
                    a_info.bitrate = int(stream.get("bit_rate", 0) or 0)
                except (ValueError, TypeError):
                    a_info.bitrate = 0
                info.audio = a_info
